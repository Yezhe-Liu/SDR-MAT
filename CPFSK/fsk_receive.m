...
% temp = iq_signal_dec.';
% for i=128:length(iq_signal_dec)
%     cor_abs_1(i)=abs((iq_signal_dec(i-127:i))*real(preamble_waveform_dec).');
%     cor_abs_2(i)=abs((iq_signal_dec(i-127:i))*imag(preamble_waveform_dec).');
% end 
...
% Demodulate signal 
[signal_bits, ~] = fsk_demod(iq_signal(sig_start_index:end), samps_per_symb, 2);
rx_signal = str2num(reshape(signal_bits',1,16)')';
rx_data_bytes =bi2de(rx_signal(1:16),'left-msb');
%Demodulate bits from the IQ signal at the start index
[bits, info.dphase] = fsk_demod(iq_signal(sig_start_index:end), samps_per_symb, rx_data_bytes + 2);
